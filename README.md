...
| `rsvp.html`      | RSVP            | Full RSVP form — name, guests, meal, dietary, song request |
...
| `messages.html`  | Messages        | Simple Formspree message form for guests |

---

## 📬 Setting Up Formspree (Messages + RSVP)

You need **two separate Formspree forms** — one for messages, one for RSVPs.
Both use the same free account (50 submissions/month each).

### Step-by-step:
1. Go to [formspree.io](https://formspree.io) → create a free account
2. Click **+ New Form** → name it **"Wedding Messages"** → copy the endpoint
3. Click **+ New Form** again → name it **"Wedding RSVP"** → copy that endpoint
4. Open `js/config.js` and paste:
   ```js
   messages: {
     formspreeEndpoint: "https://formspree.io/f/YOUR_MESSAGES_ID"
   },
   rsvp: {
     formspreeEndpoint: "https://formspree.io/f/YOUR_RSVP_ID"
   }
   ```
5. The yellow setup instructions boxes on each page will auto-hide once the endpoints are set.

**Formspree free tier:** 50 submissions/month per form. For a wedding, this is plenty — upgrade to paid ($10/mo) only if you have 50+ RSVPs.
